...
<template>
  ...
  <!-- <div class="board-detail">
    <div class="board-header">
      <h1 class="board-title">{{ this.board.boardTitle }}</h1>
      <div class="board-meta">
        <div class="board-author">작성자: {{ this.board.userId }}</div>
        <div class="board-date">작성일: {{ this.board.boardDate }}</div>
      </div>
    ...
    <h3>댓글</h3>
    <div v-if="updateCommentStatus === true" class="update-comment">
      <input v-model="updateCommentContents" />
      <div class="button-group">
        <v-btn class="update-comment-button" @click="updateComment(commentId)">댓글 수정</v-btn>
        <v-btn class="cancel-button" @click="updateCommentToUnvisible">취소</v-btn>
      </div>
    </div>
    <table id="comment">
      <thead>
    ...
    <div class="create-comment">
      <input v-model="commentContents" />
      <v-btn class="create-comment-button" v-if="!this.userIdComment" disabled>댓글 작성</v-btn>
      <v-btn class="create-comment-button" v-else @click="createComment">댓글 작성</v-btn>
    </div>
  </div> -->
</template>
<script>
